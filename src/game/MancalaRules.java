...
public class MancalaRules {
	...
	public MancalaRules(ArrayList<Integer> top, ArrayList<Integer> bottom) {
	...
	public int getBestNextMove(int searchDepth) {
		...
		int bestMove = -1;
		for (int i = 5; i > -1; --i) {
			MancalaRules testMancalaRules;

			if (turn) {
				if (topBoard.get(i) == 0)
					continue;
				testMancalaRules = MancalaRules.newGameWithMove(topBoard, bottomBoard, i);
			} else {
				if (bottomBoard.get(i) == 0)
					continue;
				testMancalaRules = MancalaRules.newGameWithMove(bottomBoard, topBoard, i);
			}

			// testMancalaRules.printBoard();
			if (testMancalaRules.turn) {
				newValue = MancalaRules.findBestNextMove(testMancalaRules.topBoard,
						testMancalaRules.bottomBoard, 50, bestValue, searchDepth);
			} else {
				newValue = -MancalaRules.findBestNextMove(testMancalaRules.bottomBoard,
						testMancalaRules.topBoard, -bestValue, -50, searchDepth);
			}
	...
	public ArrayList<Integer>setBoard() {
		...
		listBoard.addAll(topBoard);
		listBoard.addAll(bottomBoard);
		return listBoard;
	...
	public static MancalaRules newGameWithMove(ArrayList<Integer> myTopBoard,
			ArrayList<Integer> myBottomBoard, int position) {
		MancalaRules myMancalaRules = new MancalaRules(myTopBoard, myBottomBoard);
		myMancalaRules.move(position);
		return myMancalaRules;
	...
	public static double findBestNextMove(ArrayList<Integer> myBoard,
		...
		// MancalaRules.printBoardPieces(myBoard, otherBoard);
		...
		for (int i = 5; i > -1; --i) {
			...
			{
				MancalaRules nextBoard = newGameWithMove(myBoard, otherBoard, i);
				double boardValue;
		...
		public void setEnd(boolean isFinished) {
